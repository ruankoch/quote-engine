...
<style>
  ...
  *{box-sizing:border-box}
  ...
  body{
    ...
    padding-top:calc(64px + env(safe-area-inset-top)); padding-bottom:calc(52px + env(safe-area-inset-bottom));
    height:100vh; height:100dvh; overflow:hidden; transition:background .4s,color .4s;
  }
  .topbar{position:fixed; top:calc(14px + env(safe-area-inset-top)); left:0; right:0; display:flex; gap:10px; justify-content:space-between;
  ...
  .card::before{content:"\201C"; position:absolute; top:-8px; left:16px; font-size:clamp(84px,22vw,150px); line-height:1;
    color:var(--accent); opacity:.12; font-family:Georgia,serif; pointer-events:none; z-index:0;}
...
function renderAccount(){
  ...
  el('localPrefsWrap').style.display = (store.sheet||syncing) ? 'none' : '';
  if(syncing){
    pill.className='pill'; pill.textContent='connecting…';
    addPill.className='pill'; addPill.textContent='connecting…';
    body.innerHTML='<p class="note">Showing a quote from the built-in copy while it connects to your <b>Google Sheet</b>…</p>';
    return;
  }
  if(store.sheet){
...
function applyThemeColor(){ const m=document.querySelector('meta[name="theme-color"]'); if(m) m.setAttribute('content', document.documentElement.dataset.mode==='dark'?'#14110d':'#f4f1ea'); }
el('mode').addEventListener('click', ()=>{ const h=document.documentElement; h.dataset.mode=h.dataset.mode==='dark'?'light':'dark'; applyThemeColor(); });
...
// ---------- boot ----------
let syncing=false;
// Once the live Sheet has loaded, swap it in without flashing: keep the quote
// already on screen if it still exists in the Sheet (matched by text).
function afterSheetLoad(){
  store=SheetStore; syncing=false;
  refreshThemes();
  const keep = current && ITEMS.find(it=>!it.hidden && norm(it.q)===norm(current.q));
  const v=el('filter').value; const base=activeQuotes();
  pool=(v==='__all')?base:base.filter(it=>it.t===v);
  el('count').textContent=pool.length+' quotes'+(v==='__all'?' · '+THEMES.length+' themes':' in this theme')+' · ☁ Google Sheet';
  el('hiddenCount').textContent=hiddenCount();
  if(keep){ current=keep; history=[keep.id]; histPos=0; renderFreq(); updateNav(); }
  else if(!pool.length){ renderEmpty(); updateNav(); }
  else { resetHistory(); current=null; show(); }
  renderAccount();
}
async function init(){
  applyThemeColor();
  // Phase 1: paint a quote instantly from the built-in copy — no network wait.
  syncing = SHEET_CONFIGURED;
  const useSheet = SHEET_CONFIGURED;
  store = LocalStore;
  renderAccount();
  await LocalStore.load();
  refreshThemes(); applyFilter();
  // Phase 2: quietly swap in the live Google Sheet in the background.
  if(useSheet){
    try{ await SheetStore.load(); afterSheetLoad(); }
    catch(err){ syncing=false; store=LocalStore; renderAccount(); toast('Google Sheet unavailable — showing the built-in copy.'); }
  }
}
init();
</script>
